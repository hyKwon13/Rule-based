...
def mouse_callback(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if drawing:
            ...
            cv2.imshow('Rotated object detection', frame_copy)
...
def main():
    ...
    while True:
        ...
        if not roi_selected:
            cv2.imshow('Rotated object detection', frame)
            cv2.setMouseCallback('Rotated object detection', mouse_callback, param=frame)
        else:
            ...
            if success:
                cv2.putText(frame_corrected, "Rotated object detected", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            else:
                cv2.putText(frame_corrected, "Rotated object detection failed", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)

            cv2.imshow('Rotated object detection', frame_corrected)
